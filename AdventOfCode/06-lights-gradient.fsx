...
type LightLevel = | Level of int

type Coordinate = (int * int)

type Grid = (Coordinate * LightLevel) list

type Range = { From : Coordinate; To : Coordinate}
...
let parse (commandText : string) =
    commandText.Split([|" ";","|], System.StringSplitOptions.RemoveEmptyEntries)
    |> List.ofArray
    |> function 
        | "turn" :: "off" :: x1 :: y1 :: "through" :: x2 :: y2 :: [] -> TurnOff { From = (parseInt x1, parseInt y1); To = (parseInt x2, parseInt y2)}
        | "turn" :: "on" :: x1 :: y1 :: "through" :: x2 :: y2 :: [] -> TurnOn { From = (parseInt x1, parseInt y1); To = (parseInt x2, parseInt y2)}
        | "toggle" :: x1 :: y1 :: "through" :: x2 :: y2 :: [] -> Toggle { From = (parseInt x1, parseInt y1); To = (parseInt x2, parseInt y2)}
        | _ -> failwith (sprintf "Unrecognized command: %s" commandText)
...
let startingGrid : Grid = [for r in [0..999] do for c in [0..999] -> ((r,c), Level 0)]

let inRange (x,y) {From = (fx, fy); To = (tx, ty)} =
    fx <= x && x <= tx && fy <= y && y <= ty
let toggled (Level l) = Level (l+2)

let rec toggle grid range acc = 
    match grid with
    | [] -> acc
    | (coord, state) :: t when inRange coord range -> (toggle t range ((coord, toggled state) :: acc))
    | light :: t -> toggle t range (light :: acc)

let rec turnOn grid range acc = 
    match grid with
    | [] -> acc
    | (coord, (Level l)) :: t when inRange coord range -> (turnOn t range ((coord, (Level (l + 1))) :: acc))
    | light :: t -> turnOn t range (light :: acc)

let turnOneOff (Level lvl) =
    [0; lvl - 1] 
    |> List.max 
    |> Level

let rec turnOff grid range acc = 
    match grid with
    | [] -> acc
    | (coord, state) :: t when inRange coord range -> (turnOff t range ((coord, turnOneOff state) :: acc))
    | light :: t -> turnOff t range (light :: acc)

let apply grid command 
    = 
    printfn "%A" command
    match command with
        | Toggle r -> toggle grid r []
        | TurnOn r -> turnOn grid r []
        | TurnOff r -> turnOff grid r []

printfn "Let's go!"
let endState =
    parsedCommands
    |> List.fold apply startingGrid
