...
type Grid = (Coordinate * LightStatus) list
...
let startingGrid : Grid = [for r in [0..999] do for c in [0..999] -> ((r,c),Off)]
...
let rec turnOn grid range acc = 
    match grid with
    | [] -> acc
    | (coord, state) :: t when inRange coord range -> (turnOn t range ((coord, On) :: acc))
    | light :: t -> turnOn t range (light :: acc)

let rec turnOff grid range acc = 
    match grid with
    | [] -> acc
    | (coord, state) :: t when inRange coord range -> (turnOff t range ((coord, Off) :: acc))
...
let result = endState |> List.filter (fun (_, status) -> status = On) |> List.length
