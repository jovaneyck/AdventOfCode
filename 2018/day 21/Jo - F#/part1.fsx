...
let registerValue device index = 
    if index = (Register 0) then
        printfn "READING R0"
    device.registers |> Map.find index
...
let result = run [8797248;0;0;0;0;0] input //Looked at how R0 gets used, then figured out the program state when R0 gets read
