...
type Instruction = { a : int; b : int; c : Register }
...
let instruction a b c = {a = a; b = b; c = Register c}
...
let parseSample sample =
    ...
    let parseInstruction (instruction : string) =
        ...
        { a = int numbers.[1]; b = int numbers.[2]; c = Register (int numbers.[3])}
...
let tt () =
    ...
    test <@ parseSample example = { before = deviceWith [3;2;1;1]; after = deviceWith [3;2;2;1]; instruction = instruction 2 1 2 } @>
    test <@ parseSample example |> behavesLike = [Addi; Mulr; Seti] @>
    printfn "..done!"  
...
let result =
    ...
    |> List.filter (fun opcodes -> 3 <= (opcodes |> List.length ))
    |> List.length