...
let findMostSleptMinute sleeps =
...
let solve input =
    ...
    let sleepTimes = 
        actionsPerGuard
        |> List.map (fun (id, actions) -> (id, toSleepTime actions))
    let (biggestSleeperId, biggestSleeperNaptimes) = 
        sleepTimes
        |> List.maxBy (fun (_, state) -> state.sleeps |> List.sumBy (fun (start, stop) -> stop - start))
    let mostSleptMinute =
        findMostSleptMinute biggestSleeperNaptimes.sleeps

    (biggestSleeperId, mostSleptMinute, biggestSleeperId * mostSleptMinute)
...
printf "Testing..."
test <@ parseDate "1518-11-01 00:36" = new DateTime(1518,11,1,0,36,0) @>
test <@ solve example = (10, 24, 240) @>
printfn "..done!"
