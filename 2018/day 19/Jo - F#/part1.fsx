...
let deviceWith registerValues = 
    { registers = (List.zip  ([0..4] |> List.map (Register)) registerValues ) |> Map.ofList; instructionpointer = 0; boundRegister = None }
...
let parse (input :string list) = 
    ...
    ({ deviceWith [0;0;0;0;0] with boundRegister = Some boundRegister }, instructions)

let rec loop instructions device = 
    ...
    | Some i ->
        ...
        loop instructions next

let run input = 
    let device, instructions = input |> parse
    loop instructions device
...
let tt () =
    printf "Testing..."
    test <@ (run example).registers |> Map.toList |> List.map snd = [6; 5; 6; 0; 0; 9]  @> //registers at end
    printfn "..done!"  
