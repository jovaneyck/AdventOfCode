...
let rec cook recipebook stocks ingredients =
    ...
    | i :: is ->
        ...
        if stock >= i.Amount then
            ...
            let batches = (float need_to_cook) / (float recipe.Output.Amount) |> System.Math.Ceiling |> int64
...
let stock = 
    ...
    |> List.map (fun i -> (i, 0L))
    |> Map.ofList
    |> Map.add "ORE"  1000000000000L

//Manual binary search ftwftw
let ore_stock =
    cook recipebook stock [{ Amount = 3_281_820L; Chemical = "FUEL" }]
    |> Map.find "ORE"
1000000000000L - ore_stock > 0L